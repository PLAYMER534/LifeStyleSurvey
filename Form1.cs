...
namespace Assignment__LifeStyleSurvey_
{
    public partial class Form1 : Form
    {
        ...
        private void InitializeSurveyPanel()
        {
            ...
            panelSurvey.Controls.Add(CreateLabel("Rate your agreement (1 to 5): 1=Strongly Agree, 2=Agree, 3=Neutral, 4=Disagree, 5=Strongly Disagree", 20, y));
            y += 25;
  ...
  private bool ValidateRatings()
        {
            ...
            {
                if (!Enumerable.Range(0, 5).Any(j => ratingChecks![i, j].Checked)) return false;
            }
            return true;
        }

        private Label CreateLabel(string text, int x, int y) => new Label { Text = text, Location = new Point(x, y), AutoSize = true, Font = new Font("Segoe UI", 10) };
        private TextBox CreateTextBox(int x, int y) => new TextBox { Location = new Point(x, y), Width = 200 };
    }
}
